...
plot.err <- plotErrors(err, nominalQ=TRUE)
ggsave("qualplot.pdf", plot.err, device="pdf")

### PROCESS ALL SAMPLES ###
if (isTRUE(pool.method) | pool.method == "pseudo"){
  # Pool method
  cat(paste0("Denoise with pooling method = ", as.character(pool.method)))
  dds <- dada(filts, err=err, pool = pool.method, multithread=multithread,
  				HOMOPOLYMER_GAP_PENALTY=HOMOPOLYMER_GAP_PENALTY,
  				BAND_SIZE=BAND_SIZE, verbose=FALSE)
  denoisedF <- unlist(lapply(dds, getN), use.names = FALSE)
  # Make sequence table
